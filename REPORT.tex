...
\title{\textbf{Distributed Multi-Head Attention}\\[2pt]
       \large A Hybrid (Task + Data) MPI Parallelization\\[4pt]
       \normalsize Parallel \& Distributed Programming --- Course Project}
\author{Nguyen Hoang Thai\\
        \small Hanoi University of Science and Technology (HUST)}
\date{\today}
...
(Chart~C).
\item \textbf{Measurement threats to validity.} The speedup study reports rank-0
timings (the root), which understate the slowest rank's communication and, at large
$p$, flatter the computation-only number because rank~0 then holds fewer query rows;
we call this out explicitly rather than smoothing it. The Wi-Fi link is also
genuinely \emph{variable} --- the same $p{=}22$ broadcast was timed at $97$\,s and
$226$\,s on separate runs --- so single-sample communication times carry real
noise, which is why each headline size was run once with a consistent binary and the
qualitative trend (not any single point) is what we draw conclusions from. A wired
network and multi-run averaging would tighten the numbers but not change the story.
\end{itemize}

\paragraph{Future work.} Three steps follow directly from the measurements. (i)
\emph{A faster interconnect.} Every result here is throttled by Wi-Fi; repeating the
study on Gigabit Ethernet (latency $\sim\!0.1$\,ms instead of $\sim\!5$\,ms,
bandwidth $\sim\!100\times$) should move the compute/communication crossover down by
one to two orders of magnitude in $N$ and let the with-communication speedup
approach the computation-only curve. (ii) \emph{Cache/query blocking.} The kernel is
memory-bandwidth bound (Chart~F); FlashAttention-style tiling that keeps a $K,V$
block resident while sweeping a tile of queries would raise arithmetic intensity and
unlock the idle SMT threads, at the cost of the current bit-exact match. (iii)
\emph{Weighted, topology-aware distribution.} The residual heterogeneity imbalance
(Section~\ref{sec:chartc}) would close if the row split were proportional to each
node's measured throughput, and a communicator that respects the
intra-node/inter-node hierarchy (reduce within a node, then across nodes) would cut
the Wi-Fi traffic that dominates today. None of these change the parallel structure;
they tune the three quantities --- network cost, arithmetic intensity, and work
placement --- that this study has isolated and quantified.
...
sequential result within tolerance (bit-exact for the 1D, head and hybrid paths;
$\sim\!10^{-7}$ for the reordered Cannon and ring paths). On the three-node,
$22$-core Wi-Fi cluster the central finding is the gap between how the
\emph{work} scales and how the \emph{runtime} scales: at the chosen size
$N^{\star}=32768$ the computation-only speedup reaches about $\mathbf{20\times}$ on
$22$ cores, yet communication over the wireless link caps the end-to-end
(with-communication) speedup at only $\mathbf{4.4\times}$ --- and that best value is
reached at $p=8$ on a \emph{single} machine, so adding the other two nodes speeds up
the arithmetic but not the wall clock. The same fast-\code{master}/slow-network
asymmetry drives the hybrid run's load imbalance to $175.7\%$ (cut to $28.6\%$ by
switching to the rootless 1D decomposition) --- both far from and then near the
$25\%$ target. The two
algorithmic improvements address exactly these costs: the sharded \emph{ring}
path cuts cross-node communication $4\times$ and idle wait almost to zero
(Chart~E), and the rank-0-only input allocation removes the memory wall that
otherwise caps the process count. The clear lesson is that for this
communication-bound, memory-bandwidth-bound operator on a commodity network, the
decomposition that \emph{minimizes and overlaps data movement} --- not the one
that merely divides the arithmetic --- is what determines real performance.
